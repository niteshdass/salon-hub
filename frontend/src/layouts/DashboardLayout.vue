...
// Each entry carries the roles its policy allows, so the sidebar never
// offers a page the API would refuse.
const visibleNav = computed(() =>
  nav.filter((item) => !item.roles || item.roles.includes(authStore.role))
)
